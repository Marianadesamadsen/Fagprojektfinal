...
for p = 1 : numpatients
    ...
    for i = 1 : number_combinations
        % Detecting meals
        D_detected(:,i) = GRIDalgorithm_mealdetection(Gsc(:,:,p),Gmin_combinations(i,:),tau,delta_G,t_vec,Ts);

        % Computing evaluation
        [truepositive(i,p), falsepositive(i,p), falsenegative(i,p), truenegative(i,p)] = ...
            detectionrates2(stride,D(:,:,p),D_detected(:,i),Ts,idx_missed(:,p), idx_less(:,p),U(:,:,p));
        
        % Total number of detected meals for the current Gmin values.
        number_detectedmeals(i,p) = sum(D_detected(:,i));
